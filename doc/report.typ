...
Datapath 和 Control
交流的信号定义为interface，在`includes/cpu_control_signals.sv`中定义。

简单介绍一下代码中一些自己想到的地方

- 常量定义在`header.sv`中，这样方便修改
- 整个项目迁移到 SystemVerilog，使用了一些 SystemVerilog 的特性，比如
  interface
- 实现了额外的lui, auipc指令
- 实现了对\<32bit的内存读写的支持。这部分通过`mem_handler.sv`实现，该模块提供了截取和扩展32bit数据的功能

然后简单过一下这几天#strike[坐牢];制造CPU的结果吧。
...
然后是`SCPU`模块，这个模块实例化了`my_datapath`模块和`my_cpu_control`模块和`mem_handler`模块。`mem_handler`这个模块基本上就是夹在`my_datapath`和
Data memory
中间，把内存喂过来的32bit数据处理成需要的宽度和，处理datapath计算的地址，处理datapath想写入的内容。它是为了实现`lb`,`lbu`,
`lh`, `lhu`, `sb`,
`sh`指令引入的。这个模块的逻辑还是比较绕，有优化空间。也许它适合呆在`my_datapath`里面？
...
#box(image("assets/2024-12-01-23-05-12.png"))

然后大概回忆一下遇到的一些问题吧

- 首先是发现无法导入使用`.edf`文件，就是下面这些文件，这个问题卡了我好几天
  - #box(image("assets/2024-12-01-23-07-31.png"))
  - 然后助教告诉我不需要使用Lab2的soc，因为暂时不上板。不过即使这样我也不能在Lab4-1的时候使用参考的Control，#strike[所以被迫全自研]
- 接下来两个问题都出在memory上
  - 首先是跳转的时候总是把跳转的后一条执行了，然后跳到目标地址的下一条开始执行。
    … beq x15, x20, test \# jump to test addi x21, x20, 1 \# 不执行,
    x21始终为0
    ...
    - 类似上面这样的代码，`addi`指令总是被执行。然后第一个`lw`不执行。原因是instruction
      memory有一个额外的寄存器，导致一周期的延迟：
      #box(image("assets/2024-12-01-23-14-44.png"))
      如上图，总是在地址提供后的第二个posedge开始反应。检查发现是这个。
      #box(image("assets/2024-12-01-23-16-17.png"))
      取消勾选后就正常了。然后把Datamemory的这个选项也取消了下。

  - 接着是Datamemory读取的内存总是一下子就跑到`.coe`的文件的外面去了。检查发现datamemory是每32bit一个地址，而不是每字节一个地址。又有`lh`,
    `lhu`等指令的需求，就写了一个`mem_handler`模块，把这个问题完美解决了。（并不是截掉末尾两位强行align，那样只能保证`lw`,
    `sw`的逻辑）如图所示，下面的是地址，上面的是读出的数据。`0x34`已经超出`.coe`文件的范围了，因此得到`0x00000000`。
    #box(image("assets/2024-12-01-23-20-09.png"))

  - 接着还遇到一个相对比较小的问题。`sra`没有做sign
    extension，如图所示，错误的得到了`0x77778000`。
    #box(image("assets/2024-12-01-23-21-47.png"))
    正确写法见`my_ALU.sv`的`SRA`部分，也就是这样`$signed(A) >>> B`，要先做`$signed`转换才行。

  - 还有一个司空见惯的错误，经常遇到trailing
    comma，终于在vscode配置完linter后，再也没看到过类似的语法错误了。也不需要专门检查，好方便！

== 实验结果与分析
<实验结果与分析>
=== 1. 1\_datapath.S
<datapath.s>
测试如图 #box(image("assets/2024-12-01-23-28-41.png"))
#box(image("assets/lab4_2_waveform2.png"))

=== 2. 2\_more.S
<more.s>
测试如图 #box(image("assets/2024-12-01-23-30-26.png"))
#box(image("assets/2024-12-01-23-30-39.png"))
...
测试如图。结果是正确的 #box(image("assets/2024-12-01-23-34-28.png"))
